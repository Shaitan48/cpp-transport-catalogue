...
void Serialize(const transportCatalogue::Catalogue& tcat,
               const renderer::MapRenderer& renderer, const transportCatalogue::Router& router,
               std::ostream& output) {
...
void SetStopsDistances(transportCatalogue::Catalogue& tcat, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.stop_size(); ++i) {
        const serialize::Stop& stop_i = database.stop(i);
        transportCatalogue::Stop* from = tcat.FindStop(stop_i.name());
...
void AddStopFromDB(transportCatalogue::Catalogue& tcat, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.stop_size(); ++i) {
        const serialize::Stop& stop_i = database.stop(i);
...
void AddBusFromDB(transportCatalogue::Catalogue& tcat, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.bus_size(); ++i) {
        const serialize::Bus& bus_i = database.bus(i);
        std::vector<transportCatalogue::Stop*> stops(bus_i.stop_size());
        ...
        if (!bus_i.final_stop().empty()) {
            transportCatalogue::Bus* bus = tcat.FindBus(bus_i.name());
...
Deserialize(std::istream& input) {
    ...
    transportCatalogue::Catalogue tcat;
    renderer::MapRenderer renderer(GetRenderSettingsFromDB(database));
    transportCatalogue::Router router(GetRouterSettingsFromDB(database.router()));
